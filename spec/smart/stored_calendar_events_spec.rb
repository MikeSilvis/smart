...
describe StoredCalendarEvents do
  subject { StoredCalendarEvents.new('test.yml') }
  before { File.open(subject.send(:file), 'w') {|file| file.truncate(0) } }

  describe '.<<' do
    let(:event) { double(id: 123, start_time: Time.now, end_time: Time.now + 300) }
    ...
    context 'can add an arra of items' do
      let(:event_2) { double(id: 124, start_time: Time.now, end_time: Time.now + 300) }
      it { expect { subject << [event, event_2] }.to change{subject.events.size}.by(2) }
    end
  end

  describe '.current' do
    let(:event) { double(id: 321, start_time: (Time.now - 300), end_time: Time.now + 300) }
    before { subject << event }
    it { subject.current[:id].should == event.id }
  end

  describe '.delete_old_events' do
    let(:event) { double(id: 132, start_time: (Time.now - 5000), end_time: Time.now - 4000) }
    before { subject << event }
